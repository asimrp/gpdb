 ...
 *
 * brin_ao_validate.c
 *	  Opclass validator for BRIN_AO.
 *
 * Portions Copyright (c) 1996-2019, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * IDENTIFICATION
 *	  src/backend/access/brin_ao/brin_ao_validate.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * Validator for a BRIN_AO opclass.
 *
 ...
 */
bool
brin_aovalidate(Oid opclassoid)
{
	bool		result = true;
	HeapTuple	classtup;
	...
	for (i = 0; i < proclist->n_members; i++)
	{
		...
		switch (procform->amprocnum)
		{
			...
			default:
				...
				if (procform->amprocnum < BRIN_AO_FIRST_OPTIONAL_PROCNUM ||
					procform->amprocnum > BRIN_AO_LAST_OPTIONAL_PROCNUM)
				{
					ereport(INFO,
							(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
							 errmsg("operator family \"%s\" of access method %s contains function %s with invalid support number %d",
									opfamilyname, "brin_ao",
	...
	for (i = 0; i < oprlist->n_members; i++)
	{
		...
		{
			/*
			 * The set of operators supplied varies across BRIN_AO opfamilies.
			 ...
			 * cross-type operator will not get noticed; but the core BRIN_AO
		...
		/* brin_ao doesn't support ORDER BY operators */
		...
		/* Check operator signature --- same for all brin_ao strategies */
	...
	foreach(lc, grouplist)
	{
		...
		/*
		 * Some BRIN_AO opfamilies expect cross-type support functions to exist,
	...
	for (i = 1; i <= BRIN_AO_MANDATORY_NPROCS; i++)
	{
		if (opclassgroup &&
