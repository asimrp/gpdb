/*
 * brin_ao_tuple.c
 *		Method implementations for tuples in BRIN_AO indexes.
 *
 * Intended usage is that code outside this file only deals with
 * Brin_AoMemTuples, and convert to and from the on-disk representation through
 * functions in this file.
 *
 * NOTES
 *
 * A BRIN_AO tuple is similar to a heap tuple, with a few key differences.  The
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/brin_ao/brin_ao_tuple.c
 */
#include "postgres.h"
...
static inline void brin_ao_deconstruct_tuple(Brin_AoDesc *brdesc,
										  char *tp, bits8 *nullbits, bool nulls,
										  Datum *values, bool *allnulls, bool *hasnulls);


/*
 * Return a tuple descriptor used for on-disk storage of BRIN_AO tuples.
 */
static TupleDesc
brtuple_disk_tupdesc(Brin_AoDesc *brdesc)
{
	/* We cache these in the Brin_AoDesc */
...
/*
 * Generate a new on-disk tuple to be inserted in a BRIN_AO index.
 *
 * See brin_ao_form_placeholder_tuple if you touch this.
 */
Brin_AoTuple *
brin_ao_form_tuple(Brin_AoDesc *brdesc, BlockNumber blkno, Brin_AoMemTuple *tuple,
				Size *size)
{
	Datum	   *values;
	bool	   *nulls;
	bool		anynulls = false;
	Brin_AoTuple  *rettuple;
	int			keyno;
	int			idxattno;
	...
	len = SizeOfBrin_AoTuple;
	if (anynulls)
	{
		/*
		 * We need a double-length bitmap on an on-disk BRIN_AO index tuple; the
	...
	Assert((rettuple->bt_info & BRIN_AO_OFFSET_MASK) == hoff);
	...
	if (anynulls)
	{
		...
		rettuple->bt_info |= BRIN_AO_NULLS_MASK;
		 ...
		 * the sense of the att_isnull test in brin_ao_deconstruct_tuple as well.
		 */
		bitP = ((bits8 *) ((char *) rettuple + SizeOfBrin_AoTuple)) - 1;
		bitmask = HIGHBIT;
		for (keyno = 0; keyno < brdesc->bd_tupdesc->natts; keyno++)
		{
			if (bitmask != HIGHBIT)
		...
		bitP = ((bits8 *) (rettuple + SizeOfBrin_AoTuple)) - 1;
	}

	if (tuple->bt_placeholder)
		rettuple->bt_info |= BRIN_AO_PLACEHOLDER_MASK;
 ...
 *
 * This is a cut-down version of brin_ao_form_tuple.
 */
Brin_AoTuple *
brin_ao_form_placeholder_tuple(Brin_AoDesc *brdesc, BlockNumber blkno, Size *size)
{
	Size		len;
	Size		hoff;
	Brin_AoTuple  *rettuple;
	...
	rettuple->bt_blkno = blkno;
	rettuple->bt_info = hoff;
	rettuple->bt_info |= BRIN_AO_NULLS_MASK | BRIN_AO_PLACEHOLDER_MASK;
...
/*
 * Free a tuple created by brin_ao_form_tuple
 */
void
brin_ao_free_tuple(Brin_AoTuple *tuple)
{
	pfree(tuple);
}

/*
 * Given a brin_ao tuple of size len, create a copy of it.  If 'dest' is not
 ...
 * updated to match.  This avoids palloc/free cycles when many brin_ao tuples
 * are being processed in loops.
 */
Brin_AoTuple *
brin_ao_copy_tuple(Brin_AoTuple *tuple, Size len, Brin_AoTuple *dest, Size *destsz)
{
	if (!destsz || *destsz == 0)
		dest = palloc(len);
	else if (len > *destsz)
...
/*
 * Return whether two Brin_AoTuples are bitwise identical.
 */
bool
brin_ao_tuples_equal(const Brin_AoTuple *a, Size alen, const Brin_AoTuple *b, Size blen)
{
	if (alen != blen)
		return false;
...
/*
 * Create a new Brin_AoMemTuple from scratch, and initialize it to an empty
 * state.
 *
 ...
 */
Brin_AoMemTuple *
brin_ao_new_memtuple(Brin_AoDesc *brdesc)
{
	Brin_AoMemTuple *dtup;
	long		basesize;

	basesize = MAXALIGN(sizeof(Brin_AoMemTuple) +
						sizeof(Brin_AoValues) * brdesc->bd_tupdesc->natts);
	...
	brin_ao_memtuple_initialize(dtup, brdesc);

	return dtup;
}

/*
 * Reset a Brin_AoMemTuple to initial state.  We return the same tuple, for
 * notational convenience.
 */
Brin_AoMemTuple *
brin_ao_memtuple_initialize(Brin_AoMemTuple *dtuple, Brin_AoDesc *brdesc)
{
	int			i;
	char	   *currdatum;
	...
	currdatum = (char *) dtuple +
		MAXALIGN(sizeof(Brin_AoMemTuple) +
				 sizeof(Brin_AoValues) * brdesc->bd_tupdesc->natts);
	for (i = 0; i < brdesc->bd_tupdesc->natts; i++)
	{
...
/*
 * Convert a Brin_AoTuple back to a Brin_AoMemTuple.  This is the reverse of
 * brin_ao_form_tuple.
 *
 ...
 * that the given Brin_AoMemTuple matches what we need here.
 *
 * Note we don't need the "on disk tupdesc" here; we rely on our own routine to
 * deconstruct the tuple from the on-disk format.
 */
Brin_AoMemTuple *
brin_ao_deform_tuple(Brin_AoDesc *brdesc, Brin_AoTuple *tuple, Brin_AoMemTuple *dMemtuple)
{
	Brin_AoMemTuple *dtup;
	Datum	   *values;
	bool	   *allnulls;
	...
	dtup = dMemtuple ? brin_ao_memtuple_initialize(dMemtuple, brdesc) :
		brin_ao_new_memtuple(brdesc);

	if (Brin_AoTupleIsPlaceholder(tuple))
	...
	tp = (char *) tuple + Brin_AoTupleDataOffset(tuple);

	if (Brin_AoTupleHasNulls(tuple))
		nullbits = (bits8 *) ((char *) tuple + SizeOfBrin_AoTuple);
	else
		nullbits = NULL;
	brin_ao_deconstruct_tuple(brdesc,
						   tp, nullbits, Brin_AoTupleHasNulls(tuple),
...
/*
 * brin_ao_deconstruct_tuple
 *		Guts of attribute extraction from an on-disk BRIN_AO tuple.
 *
 * Its arguments are:
 *	brdesc		BRIN_AO descriptor for the stored tuple
 ...
 */
static inline void
brin_ao_deconstruct_tuple(Brin_AoDesc *brdesc,
					   char *tp, bits8 *nullbits, bool nulls,
					   Datum *values, bool *allnulls, bool *hasnulls)
{
	int			attnum;
	 ...
	 * att_isnull convention used elsewhere.)  See brin_ao_form_tuple.
	 */
