/*
 * brin_ao_inclusion.c
 *		Implementation of inclusion opclasses for BRIN_AO
 *
 * This module provides framework BRIN_AO support functions for the "inclusion"
 * operator classes.  A few SQL-level support functions are also required for
 * each opclass.
 *
 * The "inclusion" BRIN_AO strategy is useful for types that support R-Tree
 * operations.  This implementation is a straight mapping of those operations
 * to the block-range nature of BRIN_AO, with two exceptions: (a) we explicitly
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/brin_ao/brin_ao_inclusion.c
 */
#include "postgres.h"
 ...
 *
 * Procedure numbers must not use values reserved for BRIN_AO itself; see
 * brin_ao_internal.h.
 */
...
static FmgrInfo *inclusion_get_procinfo(Brin_AoDesc *bdesc, uint16 attno,
										uint16 procnum);
static FmgrInfo *inclusion_get_strategy_procinfo(Brin_AoDesc *bdesc, uint16 attno,
												 Oid subtype, uint16 strategynum);


/*
 * BRIN_AO inclusion OpcInfo function
 */
Datum
brin_ao_inclusion_opcinfo(PG_FUNCTION_ARGS)
{
	Oid			typoid = PG_GETARG_OID(0);
	Brin_AoOpcInfo *result;
	 ...
	 * to look it up again.
	 */
	result = palloc0(MAXALIGN(SizeofBrin_AoOpcInfo(3)) + sizeof(InclusionOpaque));
	result->oi_nstored = 3;
	result->oi_opaque = (InclusionOpaque *)
		MAXALIGN((char *) result + SizeofBrin_AoOpcInfo(3));
...
/*
 * BRIN_AO inclusion add value function
 *
 ...
 */
Datum
brin_ao_inclusion_add_value(PG_FUNCTION_ARGS)
{
	Brin_AoDesc   *bdesc = (Brin_AoDesc *) PG_GETARG_POINTER(0);
	Brin_AoValues *column = (Brin_AoValues *) PG_GETARG_POINTER(1);
	 ...
	 * it's not going to be used any longer.  However, the BRIN_AO framework
...
/*
 * BRIN_AO inclusion consistent function
 *
 * All of the strategies are optional.
 */
Datum
brin_ao_inclusion_consistent(PG_FUNCTION_ARGS)
{
	Brin_AoDesc   *bdesc = (Brin_AoDesc *) PG_GETARG_POINTER(0);
	Brin_AoValues *column = (Brin_AoValues *) PG_GETARG_POINTER(1);
	...
	switch (key->sk_strategy)
	{
		...
		case RTAdjacentStrategyNumber:
			 ...
			 * inclusion BRIN_AO family of opclasses, but it is possible to
...
/*
 * BRIN_AO inclusion union function
 *
 * Given two Brin_AoValues, update the first of them as a union of the summary
 * values contained in both.  The second one is untouched.
 */
Datum
brin_ao_inclusion_union(PG_FUNCTION_ARGS)
{
	Brin_AoDesc   *bdesc = (Brin_AoDesc *) PG_GETARG_POINTER(0);
	Brin_AoValues *col_a = (Brin_AoValues *) PG_GETARG_POINTER(1);
	Brin_AoValues *col_b = (Brin_AoValues *) PG_GETARG_POINTER(2);
	Oid			colloid = PG_GET_COLLATION();
	AttrNumber	attno;
	Form_pg_attribute attr;
	FmgrInfo   *finfo;
 ...
 */
static FmgrInfo *
inclusion_get_procinfo(Brin_AoDesc *bdesc, uint16 attno, uint16 procnum)
{
 ...
 */
static FmgrInfo *
inclusion_get_strategy_procinfo(Brin_AoDesc *bdesc, uint16 attno, Oid subtype,
								uint16 strategynum)
{
	InclusionOpaque *opaque;
